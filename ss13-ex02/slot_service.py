...
def get_all(request: Request, db: Session):
    ...
    slot_dto = [SlotResponse.model_validate(slot) for slot in slots]
    return standard_response(
        status_code=status.HTTP_200_OK,
        data=slot_dto,
        error=None,
        message="Lấy danh sách khoang chuồng thành công!",
        path=request.url.path
    )
def create_slot(request: Request, db: Session, slot: SlotRequest):
    ...
    try:
        ...
        return standard_response(
            ...
            data=slot_dto,
            error=None,
            message="Thêm khoang chuồng thành công!",
            path=request.url.path
        )
...
def detail_slot(request: Request, db: Session, slot_id: int):
    ...
    if slot is None:
        return standard_response(
            ...
            data=None,
            error=None,
            message="Khoang chuồng không tồn tại!",
            path=request.url.path
        )
    ...
    return standard_response(
        ...
        data=slot_dto,
        error=None,
        message="Lấy thông tin khoang chuồng thành công!",
        path=request.url.path
    )
def update_slot(request: Request, db: Session, slot_id: int, slot_update: SlotRequest):
    ...
    try:
        ...
        return standard_response(
            ...
            data=None,
            error=str(error),
            message="Cập nhật khoang chuồng thất bại!",
            path=request.url.path
        )
    return standard_response(
        status_code=status.HTTP_200_OK,
        data=slot_dto,
        error=None,
        message="Cập nhật khoang chuồng thành công!",
        path=request.url.path
    )
def delete_slot(request: Request, db: Session, slot_id: int):
    ...
    try:
        ...
        return standard_response(
            ...
            data=slot_dto,
            error=None,
            message="Xóa khoang chuồng thành công!",
            path=request.url.path
        )
    except Exception as error:
        db.rollback()
        return standard_response(
            ...
            data=None,
            error=str(error),
            message="Xóa khoang chuồng thất bại!",
            path=request.url.path
        )